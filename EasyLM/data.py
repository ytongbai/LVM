        ...
        for tokens, loss_masks, loc, index in self.parallel_example_iterator():
            ...
            while len(token_buffer) > chunk_size + 1:
                ...
                if self.device_count is not None:
                    metrics['dataset_accumulated_tps_per_device'] = (
                        accumulated_throughput / self.device_count
                    )
                    metrics['dataset_average_tps_per_device'] = (
                        average_throughput / self.device_count
                    )
                batch = {
